...
module.exports = {
  ...
  fetchAll: (params) => {
    // Get model hook
    const hook = strapi.hook[Widget.orm];
    ...
    return Widget.query(function(qb) {
      // Generate match stage.
      hook.load().generateMatchStage(qb)(Widget, filters);
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Widget.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Widget._attributes)
      .filter(attribute => attribute !== Widget.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Widget._attributes[attribute].type));
